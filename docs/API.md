...
| `/v1/check` | POST | Turnstile (required in production) | bounded hosted self-check of the caller's public address |
...
Only the exact, queryless `/v1/stats` URL is accepted. The response is
